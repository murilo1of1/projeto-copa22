...
function parseScore(score: string) {
  ...
  return {
    ...
    team_1_penalties: match[1] !== undefined ? Number(match[1]) : (match[4] !== undefined ? Number(match[4]) : undefined),
    team_2_penalties: match[4] !== undefined ? Number(match[4]) : (match[1] !== undefined ? Number(match[1]) : undefined),
  };
}
...
export default function PartidasPage() {
  ...
  const matches = selectedGroup === "all"
    ? allMatches
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white">Partidas da Copa 2022</h1>
          <p className="text-slate-400">Acompanhe todas as 64 partidas realizadas no Catar</p>
        </div>
            ...
            <DialogContent className="bg-slate-800 border border-slate-700 rounded-xl shadow-xl p-8 max-w-3xl w-full min-h-[420px]">
              <DialogHeader>
                <DialogTitle className="text-2xl font-bold text-amber-400 text-center mb-2">
                  {match.team_1} <span className="text-white font-normal">vs</span> {match.team_2}
                </DialogTitle>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                <div className="bg-slate-900 rounded-lg p-3">
                  <p className="font-semibold text-white mb-1 text-center">{match.team_1}</p>
                  <ul className="text-sm text-gray-300 space-y-1">
                    <li>Gols: <span className="font-bold text-white">{match.team_1_goals}</span></li>
                    <li>xG: <span className="font-bold text-white">{match.team_1_xg}</span></li>
                    <li>Posse: <span className="font-bold text-white">{match.team_1_poss}%</span></li>
                    <li>Chutes: <span className="font-bold text-white">{match.team_1_attempts}</span></li>
                    <li>No alvo: <span className="font-bold text-white">{match.team_1_ontarget}</span></li>
                    <li>Escanteios: <span className="font-bold text-white">{match.team_1_corners}</span></li>
                    <li>Cartões amarelos: <span className="font-bold text-white">{match.team_1_yellow_cards}</span></li>
                    <li>Cartões vermelhos: <span className="font-bold text-white">{match.team_1_red_cards}</span></li>
                  </ul>
                </div>
                <div className="bg-slate-900 rounded-lg p-3">
                  <p className="font-semibold text-white mb-1 text-center">{match.team_2}</p>
                  <ul className="text-sm text-gray-300 space-y-1">
                    <li>Gols: <span className="font-bold text-white">{match.team_2_goals}</span></li>
                    <li>xG: <span className="font-bold text-white">{match.team_2_xg}</span></li>
                    <li>Posse: <span className="font-bold text-white">{match.team_2_poss}%</span></li>
                    <li>Chutes: <span className="font-bold text-white">{match.team_2_attempts}</span></li>
                    <li>No alvo: <span className="font-bold text-white">{match.team_2_ontarget}</span></li>
                    <li>Escanteios: <span className="font-bold text-white">{match.team_2_corners}</span></li>
                    <li>Cartões amarelos: <span className="font-bold text-white">{match.team_2_yellow_cards}</span></li>
                    <li>Cartões vermelhos: <span className="font-bold text-white">{match.team_2_red_cards}</span></li>
                  </ul>
                </div>
              </div>
